...
import com.example.utils.TemplateUtil;
 ...
 */
    ...
    static {
        map.put("int",Integer.class.getSimpleName());//class对象  getSimpleName（）是获取源代码中给出的‘底层类’简称
    ...
    public String upperFirstChar(String tableName) {
        String name = StrUtil.str2hump(tableName);
        String firstChar = name.substring(0,1);
        name = name.replaceFirst(firstChar,firstChar.toUpperCase());
        return name;
    }

    /**@ClassName listBeanField
     *@Description:      获取表的对应类型
     *@Data 2019/4/8
     *Author censhaojie
     */
    @Override
     ...
     */
    @Override
    public void save(GenerateInput generateInput) {
        TemplateUtil.saveJava(generateInput);
        TemplateUtil.savaJavaDao(generateInput);
        TemplateUtil.saveController(generateInput);
        TemplateUtil.saveHtmlList(generateInput);
    }
}
